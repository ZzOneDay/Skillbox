...
public class RoadController
{
    ...
    public static void main(String[] args)
    {
        ...
        for(int i = 0; i < carsCount; i++) //Переменная класса int
        {
            Car car = Camera.getNextCar(); //Переменная класса Car
    ...
    private static int calculatePrice(Car car)
    {
        ...
        if (carHeight > passengerCarMaxHeight)
        {
            ...
            {
                price = cargoCarPrice; //Грузовое авто

            }
            //Легковой автомобиль, но большего габарита, почему?
            else {
                price = passengerCarPrice;
                //Проверка на прицеп (или я думал это багажник на крышку)
                if (car.hasVehicle) {
